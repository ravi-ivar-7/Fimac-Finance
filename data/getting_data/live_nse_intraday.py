...
class M_C_api:
    def __init__(self,symbol,resolution=1,data_from=None,data_to=datetime.now()) -> None:
        ...
        if data_from==None:
            self.data_from=self.data_to-self.delta_time*4000000 #if no start date, go back to 400 entries back
        else:
    ...
    def fetch_symbol_meta(self):
        ...
        url=f'https://priceapi.moneycontrol.com/techCharts/indianMarket/stock/symbols?symbol={self.symbol}'
        #print(url,355434)
    ...
    def fetch_intraday_data(self,countback=None): #countback is how much to go in past
        ...
        try:
            if countback==None:
                ...
                if countback>1000000:
                    countback=1000000

            resp=self.session.get(f'https://priceapi.moneycontrol.com/techCharts/indianMarket/stock/history?symbol={self.symbol_meta}&resolution={self.resolution}&from={self.data_from}&to={self.data_to}&countback={countback}')
            url=f'https://priceapi.moneycontrol.com/techCharts/indianMarket/stock/history?symbol={self.symbol_meta}&resolution={self.resolution}&from={self.data_from}&to={self.data_to}&countback={countback}'
            # print(url,33)
            data=resp.json()
            if data['s']=='no_data':
                return -1
